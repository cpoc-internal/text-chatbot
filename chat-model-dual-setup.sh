...
open_webui_container_specific_target_host=198.18.130.200   # If using Rootless Docker, this value may need to be changed to the actual target host IP address.
...
if $hugging_face_access_token; then
    HF_TOKEN=$hugging_face_access_token HF_HUB_ENABLE_HF_TRANSFER=1 hf-cli download $chat_model_1_huggingface_download_source --local-dir $HOME/ai_models/$chat_model_1_huggingface_download_local_sub_directory
    HF_TOKEN=$hugging_face_access_token HF_HUB_ENABLE_HF_TRANSFER=1 hf-cli download $chat_model_2_huggingface_download_source --local-dir $HOME/ai_models/$chat_model_2_huggingface_download_local_sub_directory
else
